              ...
              <main className="flex-1 overflow-y-auto bg-[#f8fafc] transition-all duration-300 relative scroll-smooth">
                {!isSidebarVisible && (
                  <button
                    ...
                    className="hidden md:flex fixed top-6 left-6 z-50 p-3 bg-white shadow-xl rounded-2xl text-blue-600 border border-blue-50 hover:scale-110 active:scale-95 transition-all"
                  >
                    <Menu size={20} />
                  </button>
                )}
                ...
                <div className="max-w-7xl mx-auto p-4 md:p-8 min-h-screen">
                  <AnimatePresence mode="wait">
                    <PageWrapper key={currentPage}>
                      ...
                      {currentPage === 'attendance' && profile.role === 'teacher' && <TeacherAttendance schoolId={activeSchoolId} />}
                      {currentPage === 'dashboard' && profile.role === 'teacher' && <TeacherDashboard schoolId={activeSchoolId} />}
                      {currentPage === 'marks-entry' && profile.role === 'teacher' && <TeacherMarksEntry schoolId={activeSchoolId} />}
