...
export function TeacherAttendance() {
    ...
    // NEW STATE FOR SELECTED STUDENT PROFILE
    const [viewStudent, setViewStudent] = useState<any>(null);

    useEffect(() => { if (profile?.id) fetchMyClasses(); }, [profile]);

    const fetchMyClasses = async () => {
        try {
            const { data, error } = await supabase.from('teacher_assignments').select('*').eq('school_id', profile.school_id).eq('teacher_id', profile.id);
            if (error) throw error;
            setMyClasses(data || []);
        } catch (err: any) { toast.error("Failed to load classes"); } finally { setPageLoading(false); }
    };
    ...
    const fetchRegistry = async (classDetails: any) => {
        setLoading(true);
        try {
            const { data: studentList, error: profileError } = await supabase.from('profiles').select('*').eq('school_id', profile.school_id).eq('role', 'student').eq('current_class', classDetails.class_name).eq('current_section', classDetails.section).order('full_name', { ascending: true });
            if (profileError) throw profileError;

            const { data: existingRecords, error: attError } = await supabase.from('student_attendance').select('*').eq('class_name', classDetails.class_name).eq('section', classDetails.section).eq('subject_name', classDetails.subject_name).eq('date', date);
            ...
            if (existingRecords && existingRecords.length > 0) {
                ...
                currentMeta = { status: record.is_finalized ? 'finalized' : 'saved', markedBy: record.marked_by, markedAt: record.marked_at };
            } else {
        ...
        } catch (err: any) { toast.error("Error loading data"); } finally { setLoading(false); }
    };
    ...
    const handleAction = async (action: 'save' | 'finalize' | 'delete') => {
        ...
        if (action === 'finalize' && !window.confirm("Confirm Finalize? This locks the record.")) return;
        if (action === 'delete' && !window.confirm("Delete all records?")) return;

        setIsSubmitting(true);
        try {
            if (action === 'delete') {
                await supabase.from('student_attendance').delete().match({ class_name: currentClass.class_name, section: currentClass.section, subject_name: currentClass.subject_name, date: date });
                const reset: any = {}; students.forEach(s => reset[s.id] = 'present'); setAttendanceData(reset); setMetaData({ status: 'new', markedBy: '', markedAt: null }); toast.success("Records Deleted");
            } else {
                const isFinal = action === 'finalize';
                const records = students.map(s => ({ school_id: profile.school_id, student_id: s.id, teacher_id: profile.id, class_name: currentClass.class_name, section: currentClass.section, subject_name: currentClass.subject_name, date: date, status: attendanceData[s.id] || 'present', academic_year: currentClass.academic_year || '2025-2026', is_finalized: isFinal, marked_by: profile.full_name, marked_at: new Date().toISOString() }));
                await supabase.from('student_attendance').upsert(records, { onConflict: 'student_id, subject_name, date' });
                setMetaData({ status: isFinal ? 'finalized' : 'saved', markedBy: profile.full_name, markedAt: new Date().toISOString() }); toast.success(isFinal ? "Locked!" : "Saved");
            }
    ...
    const sortedStudents = [...students].filter(s => s.full_name.toLowerCase().includes(searchQuery.toLowerCase()) || s.employee_id?.includes(searchQuery)).sort((a, b) => sortBy === 'roll' ? (a.roll_number || a.employee_id || '').localeCompare(b.roll_number || b.employee_id || '', undefined, { numeric: true }) : a.full_name.localeCompare(b.full_name));
            ...
            <div className="bg-white p-6 rounded-[30px] shadow-sm border border-slate-100 flex flex-col md:flex-row gap-6 items-end">
                <div className="flex-1 w-full space-y-2">
                    ...
                    <div className="relative">
                        ...
                        <select className="w-full pl-12 pr-4 py-4 bg-slate-50 rounded-2xl text-sm font-bold outline-none appearance-none cursor-pointer" value={selectedClassId} onChange={(e) => setSelectedClassId(e.target.value)}>
                            <option value="">-- Choose a Class --</option>
                            {myClasses.map((cls) => (<option key={cls.id} value={cls.id}>Class {cls.class_name} - {cls.section} ({cls.subject_name})</option>))}
                        </select>
                    </div>
                </div>
                <div className="w-full md:w-64 space-y-2">
                    ...
                    <div className="relative">
                        ...
                        <input type="date" className="w-full pl-12 pr-4 py-4 bg-slate-50 rounded-2xl text-sm font-bold outline-none cursor-pointer" value={date} onChange={(e) => setDate(e.target.value)} />
                    </div>
                </div>
            </div>
                ...
                <>
                    ...
                    <div className={`w-full p-5 rounded-[30px] flex flex-col md:flex-row items-center justify-between gap-6 border-l-8 shadow-sm transition-all ${metaData.status === 'finalized' ? 'bg-green-50 border-green-500 text-green-900' : metaData.status === 'saved' ? 'bg-blue-50 border-blue-500 text-blue-900' : 'bg-orange-50 border-orange-400 text-orange-900'}`}>
                        <div className="flex items-center gap-5 w-full md:w-auto">
                            ...
                            <div>
                                <h4 className="font-black text-sm uppercase tracking-widest">{metaData.status === 'finalized' ? 'Attendance Finalized' : metaData.status === 'saved' ? 'Draft Saved' : 'Attendance Not Yet Recorded'}</h4>
                            ...
                            {sortedStudents.map((student) => {
                                ...
                                return (
                                    <motion.div 
                                        ...
                                        onClick={() => setViewStudent(student)} // CLICK CARD TO OPEN PROFILE
                                    >
                                        ...
                                        <div className="grid grid-cols-3 gap-2 bg-white/60 p-1.5 rounded-[20px] border border-slate-100/50">
                                            {['present', 'absent', 'late'].map((s) => (
                                                <button 
                                                    key={s} 
                                                    disabled={isLocked} 
                                                    onClick={(e) => { e.stopPropagation(); setAttendanceData({ ...attendanceData, [student.id]: s }); }} // STOP PROPAGATION TO PREVENT MODAL OPEN
